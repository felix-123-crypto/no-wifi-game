...
function filterGames(){
  ...
  let shown=0;
  cards.forEach(card=>{const ok=(!types.length||types.includes(card.dataset.type))&&(!ages.length||ages.some(a=>card.dataset.age.includes(a)));card.hidden=!ok;if(ok)shown++});
  count.textContent=`${shown} game${shown===1?'':'s'} ready`;empty.hidden=shown!==0;
}
checks.forEach(x=>x.addEventListener('change',filterGames));
document.querySelector('#clear-filters').addEventListener('click',()=>{checks.forEach(x=>{x.checked=false});filterGames()});
